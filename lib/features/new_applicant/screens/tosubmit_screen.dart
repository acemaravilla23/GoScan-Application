...
class _ToSubmitScreenState extends State<ToSubmitScreen> {
  ...
  Future<void> _loadRequirementsStatus() async {
    try {
      ...
      print('=== LOADING REQUIREMENTS STATUS ===');
      print('User email: ${user.email}');

      final response = await ApiService.get('/scholarship/requirements-status?email=${Uri.encodeComponent(user.email)}');
      
      if (response['success'] == true) {
        print('=== REQUIREMENTS STATUS LOADED ===');
  ...
  Future<void> _downloadDocument(Future<bool> Function() downloadFunction, String documentName) async {
    ...
    try {
      ...
      if (success && mounted) {
        // Refresh requirements status after successful download
        await _loadRequirementsStatus();
  ...
  bool _getRequirementStatus(String folderTitle, String requirement) {
    ...
    // Map folder titles to database keys
    String folderKey;
            ...
            : Padding(
              ...
              Expanded(
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      // Rules and Undertaking Template
                      ...
                      // SPES Requirements Checklist Template
                      _buildTemplateSection(
                  ...
                  Expanded(
                    child: Row(
                      children: [
                        ...
                        // Download buttons for downloadable documents (except Folder 4)
