...
class _PersonalInfoScreenState extends State<PersonalInfoScreen> {
  ...
  Future<void> _uploadProfilePicture() async {
    ...
    try {
      ...
      if (response['success'] == true) {
        print('SUCCESS: Profile picture uploaded');
        ...
        final msg = response['message'] ?? 'Failed to upload picture';
        print('FAILED: $msg');
        _showError(msg);
      }
    } catch (e, stackTrace) {
  ...
  Future<void> _updatePersonalInfo() async {
    ...
    try {
      ...
      if (response['success'] == true) {
        print('SUCCESS: Personal information updated');
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          GestureDetector(
            onTap: _userProfile?['is_new_user'] == true ? null : _pickImage,
            child: Container(
              width: 120,
              height: 120,
              decoration: BoxDecoration(
          ...
          if (_selectedImage != null) ...[
            ...
            ElevatedButton.icon(
              onPressed: _userProfile?['is_new_user'] == true ? null : _pickImage,
              icon: const Icon(Icons.camera_alt, size: 20),
              label: const Text('Change Picture'),
              style: ElevatedButton.styleFrom(
                backgroundColor: _userProfile?['is_new_user'] == true ? Colors.grey[400] : const Color(0xFF2563EB),
                foregroundColor: Colors.white,
                padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12),
                shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(8),
            ...
            if (_userProfile?['is_new_user'] == true) ...[
              const SizedBox(height: 8),
              Text(
                'Profile picture upload will be available after submitting your scholarship application',
                style: TextStyle(
                  fontFamily: 'Inter',
                  fontSize: 12,
